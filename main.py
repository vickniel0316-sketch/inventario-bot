...
def construir_indice():
    ...
    except Exception as e: print(f"Error índice: {e}")
...
def cmd_pedidos(m):
    try:
        ...
        for row in data_cache.values():
            ...
            st, cons, t, u, dias = num(row[1]) or 0, num(row[8]) or 0, num(row[9]) or 0, num(row[10]) or 1, num(row[7]) or 0
            inc, cajas = False, 0
            if dias <= 3:
                if (st/u) < 5: cajas, inc = math.ceil(5 - (st/u)), True
            elif cons > 0:
                reorden = (cons * t) + (cons * 2)
                if st <= reorden:
                    cajas, inc = max(1, math.ceil(((reorden + cons*5) - st) / u)), True
            if inc:
                txt += f"{'🚨' if dias <= 3 else '⚠️'} {row[0]} → {cajas} cajas\n"; hay = True
        bot.reply_to(m, txt if hay else "✅ Todo al día", parse_mode="Markdown")
    except Exception as e: bot.reply_to(m, f"❌ Error en Pedidos: {e}")
...
def cmd_movimientos(m):
    try:
        ...
        if cant is None: bot.reply_to(m, "❌ Cantidad inválida."); return
...
def ejecutar_mov(m, fila, tipo, cant):
    try:
        ...
        nombre = f_data[0]
        val = abs(cant) if tipo == "entrada" else -abs(cant) if tipo == "salida" else cant - (num(f_data[1]) or 0)
        ahora = datetime.now(ZoneInfo("America/Santo_Domingo")).strftime("%Y-%m-%d %H:%M:%S")
        mov.append_row([ahora, nombre.lower(), tipo.capitalize(), float(val), m.from_user.first_name], value_input_option="USER_ENTERED")
        bot.reply_to(m, f"✅ {tipo.capitalize()} de *{nombre}* registrada.")
        invalidar_indice()
...
def manejador_pasos(m):
    ...
    if cid in estado:
        ...
        if d["modo"] == "editar":
            ...
            if p == "nombre": d["n"], d["paso"] = m.text.strip(), "stock"; bot.reply_to(m, "📦 Stock:")
            ...
            elif p == "seccion": d["se"], d["paso"] = m.text.strip(), "t"; bot.reply_to(m, "🚚 Tiempo:")
            elif p == "t":
                val = num(m.text)
                if val is None: bot.reply_to(m, "❌ Número requerido:"); return
                ...
                try:
                    ...
                    f_di = f'=SI.ERROR(MIN(6, ENTERO(HOY()) - ENTERO(MIN(FILTER(Movimientos!A:A, Movimientos!B:B = A{fila})))), 0)'
                    f_co = f'=SI.ERROR(ABS(SUMAR.SI.CONJUNTO(Movimientos!D:D, Movimientos!B:B, MINUSC(A{fila}), Movimientos!C:C, "Salida")) / H{fila}, 0)'
                    stock.append_row([d['n'], f_st, d['ni'], d['pa'], d['la'], d['se'], m.text.strip(), f_di, f_co, d['t'], d['u']], value_input_option="USER_ENTERED")
                    ahora = datetime.now(ZoneInfo("America/Santo_Domingo")).strftime("%Y-%m-%d %H:%M:%S")
                    mov.append_row([ahora, d['n'].lower(), "Entrada", float(abs(d['s'])), f"Sist ({m.from_user.first_name})"], value_input_option="USER_ENTERED")
                    estado.pop(cid); invalidar_indice(); bot.reply_to(m, "✅ Creado.")
                except Exception as ex: bot.reply_to(m, f"❌ Error: {ex}")
...
bot.remove_webhook()
while True:
    try: bot.polling(none_stop=True, interval=1, timeout=60)
